...
async def decide(approval_id:UUID,data:ApprovalDecision,db:AsyncSession=Depends(get_db),user=Depends(current_user)):
    ...
    approval.comment=data.comment; approval.decided_by=UUID(user["sub"]); await db.commit(); return {"id":str(approval.id),"status":approval.status,"resume_required":True}
