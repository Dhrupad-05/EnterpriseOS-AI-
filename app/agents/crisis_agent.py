...
class CrisisAgent(Agent[BusinessEventInput, CrisisRecommendation]):
    ...
    async def run(self,value):
        p=value.payload; event=value.event_type.lower(); severity=int(p.get("severity_level",5 if value.severity=="critical" else 4 if value.severity=="high" else 3)); daily_loss=float(p.get("daily_loss",p.get("revenue_at_risk",0)) or 0); employees=int(p.get("employees_affected",0) or 0)
        alternatives=[]
        ...
        return CrisisRecommendation(action="Activate crisis continuity plan",rationale=f"Severity {severity}/5; estimated exposure ${daily_loss:,.0f}/day.",confidence=.9 if daily_loss else .72,estimated_cost=float(p.get("recovery_cost",0) or 0),alternatives=alternatives,risk_flags=["employee-safety"] if employees else [],severity_level=max(1,min(5,severity)),impact_analysis={"daily_loss":daily_loss,"employees_affected":employees,"products_affected":p.get("products_affected",[])},recovery_timeline_days=int(p.get("recovery_days",2 if severity>=4 else 5)),teams_to_activate=teams)
