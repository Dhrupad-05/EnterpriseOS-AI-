...
class WorkflowOrchestrator:
    ...
    def build(self, checkpointer=None):
        ...
        async def specialists(state):
            event=BusinessEventInput.model_validate(state["event"]); name="crisis" if event.event_type.lower() in {"factoryfire","cyberattack","supplierbankruptcy","poweroutage","machinefailure","vendorbankruptcy"} else "procurement" if event.event_type.lower() in {"purchaserequest","vendordelay"} else "operations"; result=await self.agents.get(name).execute(event); return await persist(state,{"status":"awaiting_approval","recommendations":{name:result.model_dump() if hasattr(result,"model_dump") else result}})
        ...
        async def audit(state): return await persist(state,{"status":"completed","audit":[*state.get("audit",[]),{"action":"completed","snapshot":dict(state)}]})
